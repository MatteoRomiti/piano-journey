    ...
    getTemplateAjax('templates/started.hbars.html', function(tmpl) {
        loadYAML("started", function(yamlUnfiltered) {
            isComplete = function(book) {
                return book.started instanceof Date;
            }
            yaml = yamlUnfiltered.filter(isComplete);
            $("#started-count").append(yaml.length);
            ...
            if (isPermalink) {
                ...
                $("#cHead").hide();
                $("h2").hide();
                $("#started").prepend('<br>'); // TODO: Add this to CSS.
                $(".permalink").hide();
                $("li").hide();
            }
        });
...
function loadTimeline(started_yaml, timeline_body_tmpl) {
    var books = []
    $.each(started_yaml, function(idx, book) {
        book['idx'] = idx;
        books.push({
